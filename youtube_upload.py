...
# Minimum hours that must pass between two uploads. 24 => at most one per day.
# Set to 168 to enforce "at most once per week".
MIN_HOURS_BETWEEN = float(os.environ.get("YT_MIN_HOURS_BETWEEN", "24"))
...
def upload(service, path: Path, title: str) -> str:
    ...
    body = {
        "snippet": {
            "title": title,
            "description": "Upload automático (privado). Título e thumbnail "
                           "definidos manualmente. navylily.tv",
